...
想先确认装好了没有，点设置页的三份演示（**私聊·有情绪 / 私聊·日常 / 群聊**）任意一份：它用一段内置的虚构对话把整条链路跑一遍，不碰任何真实聊天记录，也方便自己截图发帖。三份分别对应判红、判绿和带署名的群聊，所以要验证球上的颜色分档有没有做对，各点一遍就清楚了。
...
- **修掉一处卡片自相矛盾。** 标题和正文里有两处会无条件说「在等你答复」，完全没看 `awaiting`。实测「明天那家店还去吗……那就这么定了」这段，对方已经收尾、`awaiting` 只有 12%，卡片却还在催「给个准话，别拖着」，正文也写着「在等你答复」——而紧挨着的依据行写的是「对方没在等」。现在两处都要求 `awaiting >= 0.5`，不成立时标题落到策略分支、正文改说「是在跟你约时间」。
- **复制的内容也带上把握度。** 离开卡片之后，「先听完，别讲道理」和「先听完，别讲道理（把握 55%）」是两条不同的信息。
- **标题右边挂着这一句的「把握」。** 那个数取自标题**实际依据的那一项**——从意图推的标题报意图的置信度，从策略推的报策略的置信度，而不是随手挑一个判定项顶上。`score` 类型的「语气放软一点」本来就没有置信度这回事，那几条不给数字。实测里有意思的一例：标题说「先听完，别讲道理」，把握只有 **55%**，因为模型对「对方是在倾诉还是抱怨」并不确定——这件事以前完全看不出来。
- **依据行是「人话 + 数字」并排，数字必须留。** 每一项都带上 Jev 给的数：`对方在等你回 88% · 这句回不好容易变味 89% · 情绪 1.56/3 · 读到 11 条`。**没有这些数，卡片就退化成一段普通的沟通建议**——「先听完，别讲道理」跟网上任何一篇技巧文章都分不出彼此；有数字才看得出它是模型从概率里读出来的结论，而不是谁写的经验之谈。但数字得跟人话并排：只说「踩雷风险 88%」没人知道踩的是什么雷，只说「这句回不好容易变味」又丢了 88%。真正该去掉的是「对方意图 倾诉情绪」那种既没有数、也没说人话的纯术语。正文里「让对方的感受有落点」也改成了「让他知道你听进去了」。
- **长按卡片复制的是完整的一整块**：判定、建议、依据、来源和时刻。原来只有标题加正文，贴进笔记或发给别人就没有来处了——从哪判的、什么时候判的全丢；上下文不足时更糟，标题是「只读到 2 条，判定不可靠」，复制出来成了"别信"紧跟一段照样给出的建议，读的人不知道那是警示。
- **卡片按钮改为「重判」和「收起」。** 收起每次都要用，复制退成**长按整张卡片**（低频动作退到手势）。注意长按要落在卡片正文区，标题行是拖动把手。
- **收起后球回到卡片的位置**并写回偏好，所以下次展开还在原处，不会跳回旧坐标。

### v1.8.0 — 卡片上多了一行「读到了什么」

<sub>这一版没有单独发过，内容含在 v1.9.0 里。</sub>
