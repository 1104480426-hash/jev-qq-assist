...
![判定结果：默认胶囊 / 点开详情](docs/screenshot.png)
...
交互闭环是三个层次：**悬浮球 = 开/关，胶囊 = 摘要，卡片 = 详情**。

点球出结果，默认收成贴在球旁边的一条胶囊，几乎不占聊天区。点胶囊展开完整判定；卡片上的「收起」退回胶囊，再点球则全部关掉。卡片可以按住顶部拖走，位置会记住；球拖到哪就停在哪，松手自动吸到最近的侧边。判定在跑的时候标题会动，因为本地模型首次加载要几秒。
